...
function obtemDados(collection) {
  ...
  firebase.database().ref(collection).on('value', (snapshot) => {
    ...
    cabecalho.insertCell().textContent = 'Nome do Projeto'
    cabecalho.insertCell().textContent = 'Tipo de Projeto'
    ...
    snapshot.forEach(item => {
      ...
      novaLinha.insertCell().textContent = item.val().nomeProjeto
      novaLinha.insertCell().textContent = item.val().tipoProjeto
  ...
  firebase.database().ref(db).on('value', (snapshot) => {
    snapshot.forEach(item => {
      if (item.ref.path.pieces_[1] === id) {
        ...
        document.getElementById('nomeProjeto').value = item.val().nomeProjeto
        document.getElementById('nomeProjeto').value = item.val().tipoProjeto
...
function salvar(event, collection) {
  ...
  else if (document.getElementById('nomeProjeto').value === '') { alert('⚠️É obrigatório informar o nome do projeto!') }
  else if (document.getElementById('tipoProjeto').value === '') { alert('⚠️É obrigatório informar o tipo de projeto!') }
